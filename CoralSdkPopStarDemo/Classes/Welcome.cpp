...
bool CWelcome::init()
{
    ...
    this->setKeyboardEnabled(true);
 	...
 	auto bg = Sprite::create(WELCOME_BG);
    bg->setAnchorPoint(Point::ZERO);
	bg->setPosition(Point::ZERO);
	this->addChild(bg, 0);

	auto logo = Sprite::create(WELCOME_LOGO);
	logo->setPosition(deGameManager->getWinSize().width/2 ,deGameManager->getWinSize().height/2+200);
	this->addChild(logo,1);

    auto QQItem = MenuItemImage::create(WELCOME_BT_QQ_NORALIMAGE,WELCOME_BT_QQ_SLECTEDIMAGE,
                                        CC_CALLBACK_1(CWelcome::BtCallback, this));
